...
class RegionSelect extends Component {
  ...
  onComponentMouseTouchDown(event) {
    ...
    const xPc =
      (clientPos.x - imageOffset.left) / this.refs.image.offsetWidth * 100;
    const yPc =
      (clientPos.y - imageOffset.top) / this.refs.image.offsetHeight * 100;
  ...
  onRegionMoveStart(event, index) {
    ...
    const regionLeft =
      currentRegion.x / 100 * this.refs.image.offsetWidth + imageOffset.left;
    const regionTop =
      currentRegion.y / 100 * this.refs.image.offsetHeight + imageOffset.top;
    const regionWidth = currentRegion.width / 100 * this.refs.image.offsetWidth;
    const regionHeight =
      currentRegion.height / 100 * this.refs.image.offsetHeight;
